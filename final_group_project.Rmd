...
card_fraud_by_state <- card_fraud %>%
...
print(card_fraud_by_state)
ggplot(card_fraud_by_state, aes(x = reorder(state, -total), y = total)) +
...
card_fraud_by_category <- card_fraud %>%
...
print(card_fraud_by_category)

ggplot(card_fraud_by_category, aes(x = reorder(category, -total), y = total)) +
  geom_bar(stat = "identity", fill="blue") +
  geom_text(aes(label=total),position=position_stack(vjust=0.5)) +
  xlab("State") +
  ylab("Category") +
