...
author: "Jenna Thomas, Colby Richardson, Jamie McGraw (Group 12)"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
card_fraud <- read_csv("card_fraud.csv") %>% 
...
sapply(card_fraud, class)

# select a smaller subset
my_card_is_fraud <- card_fraud %>% 
  # select a smaller subset, 20% of the entire dataframe 
  slice_sample(prop = 0.2) 

#observations: 
#1) low correlation between variables suggests these are quite independent variables 
#2) distributions: amt is skewed left (more large transactions), hour disproportionately in the middle of the night, slightly less fraud in the middle of the week
#3) other: amt clusters below around 300 and just below 1000 
my_card_is_fraud %>% 
  filter(is_fraud == 1) %>% 
  select(amt,hour,wday,age,distance_km)%>% 
  ggpairs(alpha=0.5)+
  theme_bw() +
  theme(axis.text = element_text(size = 5))


my_card_no_fraud <- card_fraud %>% 
  # select a smaller subset, 1% of the entire dataframe 
  slice_sample(prop = 0.001) 

##observations: 
#1) low correlation between variables suggests these are quite independent variables
#2) distributions: AMT is skewed right (more small transactions), HOUR: more evenly spread from afternoon to night, WDAY: more drastic concentration of transactions towards beginning of the week, AGE: more right skewed than fraud data (i.e. older people more vulnerable to fraud)
#3) other: amounts > 250 very uncommon
my_card_no_fraud %>% 
  filter(is_fraud == 0) %>% 
  select(amt,hour,wday,age,distance_km)%>% 
  ggpairs()+
...
# assess whether there's significant difference between amount of fraud in 2019 vs. 2020
card_fraud %>%
  group_by(is_fraud,year) %>%
  summarise(amt) %>%
  mutate(year = as.factor(year)) %>%
  ggplot(aes(x = is_fraud, y = amt, fill = year)) +
  geom_bar(stat = "identity", position = "dodge")
# doesn't seem to be a difference between the years


# assess fraud by state  
...
card_fraud %>%
  ...
  xlab("State") +
  ylab("Incidents of Fraud") +
  ggtitle("Top 10 States for Fraud (2019-2020)") +
  theme_minimal()

# assess fraud by category
card_fraud %>%
  ...
  xlab("State") +
  ylab("Category") +
  ggtitle("Top 5 Categories for Fraud (2019-2020)") +
  theme_minimal()
```
...
# did not end up needing this data
...
Thus, we will work with a smaller sample of 10% of the values the original dataset to identify the best model, and once we have the best model we can use the full dataset to train- is_fraud our best model.
...
my_card_fraud <- card_fraud %>% 
  ...
  select(is_fraud, amt, distance_km, hour, wday, month_name, age, category) %>%
  # select a smaller subset, 5% of the entire dataframe 
  slice_sample(prop = 0.05) 
```


## Split the data in training - is_frauding
...
# card_fraud_is_fraud <- is_frauding(data_split)
```
...
```{r, define_recipe,error=TRUE}

fraud_rec <- recipe(is_fraud ~ ., data = card_fraud_train) %>%
  # Transforms data to remove right-skewed variables
  step_log(amt, distance_km, age) %>%
...
## `last_fit()` on is_fraud set

# - `last_fit()`  fits a model to the whole training data and evaluates it on the is_fraud set. 
...
# Boosted has highest mean ROC and quickest processing time
...
cost <- cost %>%
  mutate(
    #calculate dollar costs for below equation to compare navie vs our model
    ...
    #so we cab check that our definitions match our confusion matrix above
    true_positives_count=if_else((is_fraud ==1)&(pred ==1),1,0),
    true_negatives_count=if_else((is_fraud ==0)&(pred ==0),1,0),
    false_positives_count=if_else((is_fraud ==0)&(pred ==1),1,0),
    false_negatives_count=if_else((is_fraud ==1)&(pred ==0),1,0)
  )
# Summarising - cost have been calculated and our true/false positive/negative counts map to our confusion matrix
cost_summary <- cost %>% 
  summarise(across(starts_with(c("false","true", "amt")), 
            ~ sum(.x, na.rm = TRUE)))

#calculating our model improvement = ~$1.5m
